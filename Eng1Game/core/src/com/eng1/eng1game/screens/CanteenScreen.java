...
public class CanteenScreen implements Screen {
    ...
    public void render(float delta) {
        ...
        if(SPRITE_LOCATION_X>0 && SPRITE_LOCATION_X<150){
            game.batch.draw(SpaceButton, 100, 0 , SPACEBUTTON_WIDTH,SPACEBUTTON_HEIGHT);
            game.batch.draw(EatButton, 100, 0 + SPACEBUTTON_HEIGHT, SPACEBUTTON_WIDTH, SPACEBUTTON_HEIGHT);
        ...
        if(SPRITE_LOCATION_X>-300 && SPRITE_LOCATION_X<-100) {
            ...
            game.batch.draw(EatButton, -150, 0 + SPACEBUTTON_HEIGHT, SPACEBUTTON_WIDTH, SPACEBUTTON_HEIGHT);
            if (Gdx.input.isKeyPressed(Input.Keys.SPACE)) {
                if(game.energy < 0.75) {
                    game.energy += 0.25;
                }

...
public class DormRoomScreen implements Screen {
    ...
    public void render (float delta) {
        ...
        if(SPRITE_LOCATION_X>-300 - SPRITE_WIDTH && SPRITE_LOCATION_X <-100){
            game.batch.draw(SpaceButton, -200, SPRITE_HEIGHT, game.BUTTON_WIDTH,game.BUTTON_HEIGHT);
            game.batch.draw(SleepWord, -200, SPRITE_HEIGHT + game.BUTTON_HEIGHT, game.BUTTON_WIDTH,game.BUTTON_HEIGHT);
        ...
        if(SPRITE_LOCATION_X>200 && SPRITE_LOCATION_X <300){
            game.batch.draw(SpaceButton, 250, SPRITE_HEIGHT, game.BUTTON_WIDTH,game.BUTTON_HEIGHT);
            game.batch.draw(StudyWord, 250, SPRITE_HEIGHT + game.BUTTON_HEIGHT, game.BUTTON_WIDTH,game.BUTTON_HEIGHT);
            if(Gdx.input.isKeyJustPressed(Input.Keys.SPACE)){
                game.study += 4;
                game.energy -= 0.25;
                game.worldTimer-=32;
